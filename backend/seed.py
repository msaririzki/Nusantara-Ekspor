...
def seed_data():
    ...
    with SessionLocal() as session:
        ...
        admin = session.execute(select(User).where(User.email == 'admin@ekspor.id').limit(1)).scalars().first()
        if not admin:
            admin = User(
                email="admin@ekspor.id",
                hashed_password=hash_password("admin123"),
                full_name="Admin Utama",
                company_name="Nusantara Ekspor",
                role="admin",
                country="Indonesia",
                phone="+628000000000",
        ...
        umkm_user = session.execute(select(User).where(User.email == 'umkm@ekspor.id')).scalars().first()
        if not umkm_user:
            umkm_user = User(
        ...
        buyer_user = session.execute(select(User).where(User.email == 'buyer@global.com')).scalars().first()
        if not buyer_user:
            buyer_user = User(
        ...
        session.commit()
        print("[SUCCESS] Data seeder berhasil dimasukkan ke database!")
